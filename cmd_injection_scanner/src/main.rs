    ...
    match login(&client, base_url) {
        Ok(Some(success)) => {
            ...
            match cmd_injection {
                ...
                None => println!("No Command Injection found"),
            }
        }
...
fn command_injection_scanner(
    ...
    let mut cmd_params: HashMap<&str, &str> = HashMap::new();
    ...
    let cmd_injection_response_html_response = cmd_injection_response.text().unwrap();
    if cmd_injection_response_html_response.contains("index.php") {
        Ok(Some(format!("found command injection in ip parameter at this URL {}", full_url)))
    } else {
        Ok(None)
    }
