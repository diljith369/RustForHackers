...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match login(&client, base_url) {
        Ok(Some(success)) => {
            ...
            let xss_stored = xss_stored_scanner(&client, base_url)?;
            match xss_stored {
                Some(xss) => println!("XSS Stored {}", xss),
                None => println!("No XSS Stored"),
            }
        }
